...
class MyDocumentsController(
    ...
    fun updateDocumentsFromResponse(response: DocumentListResponse) {
        Log.d(
            ...
            "Processing document response with ${response.personalDoc?.size ?: 0} personal docs and ${response.professionalDoc?.size ?: 0} professional docs",
        )
        ...
        response.personalDoc?.forEachIndexed { index, doc ->
            Log.d(
                "MyDocumentsController",
                "Processing personal doc[$index]: docName=${doc.docName}, doc_type=${doc.doc_type}, filePath=${doc.filePath}",
            )
        ...
        response.professionalDoc?.forEachIndexed { index, doc ->
            Log.d(
                "MyDocumentsController",
                "Processing professional doc[$index]: docName=${doc.docName}, doc_type=${doc.doc_type}, filePath=${doc.filePath}",
            )
    ...
    fun onViewClick(
        ...
        if (rawFilePath.isNullOrEmpty()) {
            android.widget.Toast
                .makeText(
                    context,
                    "No document found for $documentName. Please upload document for the same.",
        ...
        if (formattedUrl.isNullOrEmpty()) {
            ...
            Log.e(
                "MyDocumentsController",
                "onViewClick: Invalid or empty URL after validation for $documentName. Original path: $rawFilePath",
            )
            return
        }
        ...
        Log.d(
            "MyDocumentsController",
            "onViewClick: Document '$documentName' - Formatted URL: '$formattedUrl', isPdf: $isPdf, isImage: $isImage",
        )
