    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        val skipAuth =
            url.contains("send-otp") ||
                url.contains("otpVerify") ||
                url.contains("otp") ||
                url.endsWith("/otpVerify") ||
        ...
        val isOtpEndpoint =
            url.contains("otp", ignoreCase = true) ||
        ...
        if (response.code == 401 && !skipAuth && !isOtpEndpoint) {
            ...
            Log.d(
                "AuthInterceptor",
                "Received 401 for endpoint, not handling as token expiration (skipAuth: $skipAuth, isOtpEndpoint: $isOtpEndpoint): $url",
            )
        }
